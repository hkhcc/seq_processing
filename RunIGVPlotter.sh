...
$PATH_TO_IGVPLOTTER --igv-jar-path $PATH_TO_IGV -p SAM.SHOW_CENTER_LINE=TRUE -p SAM.ALLELE_THRESHOLD=0.005 -p SAM.ALLELE_USE_QUALITY=FALSE -p SAM.SHOW_SOFT_CLIPPED=TRUE -p SAM.COLOR_BY=READ_STRAND -m 8G --squish -v -L $PATH_TO_TEMP_FILE -o $1 $PATH_TO_SAMPLE $PATH_TO_CONTROL $PATH_TO_VCF $PATH_TO_SV_VCF $PATH_TO_ROI
